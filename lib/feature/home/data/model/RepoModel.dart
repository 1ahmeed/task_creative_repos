...
class RepoModel extends RepoEntity{
  RepoModel({
      ...
      this.openIssues, 
      this.watchers, 
      this.defaultBranch,}):super(
    descriptionRepo: description??" no description",
    nameRepo:fullName??"no name repo" ,
    userNameOfRepo:owner!.login ??" no name user",
    htmlUrl: htmlUrl ??"",
    isFork: fork ?? false
  );
    ...
    webCommitSignoffRequired : json['web_commit_signoff_required'], topics: json["topics"] == null ? [] : List<dynamic>.from(json["topics"]!.map((x) => x)), visibility : json['visibility'],
    forks : json['forks'],
